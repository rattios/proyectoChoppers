	...
	allState(ev) {
		...
		if (ev) {
		  for (var i = 0; i < this.estados.length; ++i) {
		    ...
		    this.selected_estados.push(this.estados[i]);
		    this.setMunicipios(this.estados[i].id, false);
		  }
		} else {
		  for (var i = 0; i < this.estados.length; ++i) {
		    this.estados[i].checked = false;
		    this.setMunicipios(this.estados[i].id, true);
		  }
		}
	}
